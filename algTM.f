      ...
      DOUBLE PRECISION::w,Ref,Tra,Ang,m2,At,Lt,fi,ff
      complex*16::TrM
      integer::m,j,g,l,mm,nang,ran_fre,Nf,NTP
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &,status="replace")
      ...
      MA(1,1)=Qj(per,par,Npp,anc,j,w,Ang)/eps(per,par,Npp,anc,j,w)
     &+Qair(w,Ang)
      MA(1,2)=Qj(per,par,Npp,anc,j,w,Ang)/eps(per,par,Npp,anc,j,w)
     &-Qair(w,Ang)
      MA(2,1)=MA(1,2)
      MA(2,2)=MA(1,1)
      ...
      MAI(1,2)=-MA(1,2)
      MAI(2,1)=-MA(1,2)
      MAI=0.25*(eps(per,par,Npp,anc,j,w)/(Qj(per,par,Npp,anc,j,w,Ang)*
     &Qair(w,Ang)))*MAI
